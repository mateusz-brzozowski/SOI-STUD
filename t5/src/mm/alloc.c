...
#define FIRST_FIT_ALG 0
#define WORST_FIT_ALG 1
...
PRIVATE int alloc_alg = FIRST_FIT_ALG;
...
PUBLIC phys_clicks alloc_mem(phys_clicks clicks) {
  register struct hole *hp, *max_ptr, *prev_ptr, *max_prev_ptr;
  ...
  case WORST_FIT_ALG:
    ...
    hp = hole_head;
    while (hp != NIL_HOLE && hp->h_base < swap_base) {
      if (hp->h_len >= clicks && (max_ptr == NIL_HOLE || hp->h_len > max_ptr->h_len)) {
    ...
    if (max_ptr != NIL_HOLE) {
      ...
      if (max_ptr->h_len == 0) del_slot(max_prev_ptr, max_ptr);
      return old_base;
    }
    } while (swap_out());
  default:
...
PUBLIC int do_hole_map() {
  ...
  while (hp != NIL_HOLE && hp->h_base < swap_base && i < 1022) {
    buffer[i++] = hp->h_len;
    buffer[i++] = hp->h_base;
    hp = hp->h_next;
  }
  ...
  buffer[i] = 0;
  sys_copy(MM_PROC_NR, D, buffer, who, D, mm_in.m1_p1, mm_in.m1_i1);
  return i/2;
}

PUBLIC int do_worst_fit() {
  if (mm_in.m1_i1 == 0 || mm_in.m1_i1 == 1){
    alloc_alg = mm_in.m1_i1;
  }
  return 0;
}
