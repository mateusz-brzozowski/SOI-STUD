...
FORWARD _PROTOTYPE( unsigned int get_p_group, (message *m_ptr) );
FORWARD _PROTOTYPE( unsigned int set_p_group, (message *m_ptr) );
...
/* Main entry point of sys_task.  Get the message and dispatch on type. */
  ...
  while (TRUE) {
	...
	switch (m.m_type) {	/* which system call */
      ...
      case SYS_GETPGROUP:  r = get_p_group(&m); break;
      case SYS_SETPGROUP:  r = set_p_group(&m); break;
	    default:		r = E_BAD_FCN;
	}
...
/* Handle sys_fork().  m_ptr->PROC1 has forked.  The child is m_ptr->PROC2. */
  ...
  *rpc = *rpp;			/* copy 'proc' struct */
  ...
  rpc->p_group = 0; /* set default group */
...
PRIVATE unsigned int get_p_group (m_ptr) message *m_ptr;
{
  struct proc* proces = proc_addr(m_ptr->m1_i1);
  return proces->p_group;
}

PRIVATE unsigned int set_p_group (m_ptr) message *m_ptr;
{
  unsigned int dest_proces_group = m_ptr->m1_i2;
  struct proc* proces = proc_addr(m_ptr->m1_i1);
  if(dest_proces_group == 0 || dest_proces_group == 1)
  {
    proces->p_group = dest_proces_group;
    return proces->p_group;
  }
  return ENOENT;
}
